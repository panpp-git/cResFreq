...
def load_dataloader(num_samples, signal_dim, max_n_freq, min_sep, distance, amplitude, floor_amplitude,
    ...
    clean_signals, f, nfreq ,r= data.gen_signal(num_samples, signal_dim, max_n_freq, min_sep, distance=distance,
                                              amplitude=amplitude, floor_amplitude=floor_amplitude,
                                              variable_num_freq=True)
    frequency_representation,fr_ground,m1,m2 = fr.freq2fr(f, xgrid, kernel_type, kernel_param,r,nfreq)
    ...
    dataset = data_utils.TensorDataset(clean_signals, frequency_representation, f,fr_ground,m1,m2)
...
def load_dataloader_fixed_noise(num_samples, signal_dim, max_n_freq, min_sep, distance, amplitude, floor_amplitude,
    ...
    m1 = torch.from_numpy(m1).float()
    m2 = torch.from_numpy(m2).float()
    dataset = data_utils.TensorDataset(noisy_signals, clean_signals, frequency_representation, f,fr_ground,m1,m2)
